...
% AMY I would include specific numbers as opposed to qualitative statements (high and low).  For example turn on vitro is often 1000x for the best dye binding aptamers but cellular constrast is typically 2-4x
...
\comment{Discuss RNA Fish here??} % AMY Hard to say - FISH is very powerful and certainly amenable to single molecule detection.  But is limited to fixed cells so suboptimal for live cell tracking.  To get around having to specifically discuss FISH, you could emphasize earlier that these are tools for live cell tracking.
...
% AMY This may be a subtle distinction, but the dye binding aptamers were evolved in vitro (using SELEX) to bind a dye.  So they are aptamers but not riboswitches.  I think one of the strengths of using a riboswitch as the aptamer / RNA tag portion is that it was evolved in cells by natural evolution so maintains robust folding and specific binding under chemical conditions (temperature, salt, and different RNA contexts) found in a cell.
...
Though promising, the initial iteration of Riboglow has significant room for improvement. With initial constructs, signal induction upon aptamer binding never exceeded seven fold.
% AMY and this is in vitro
...
% AMY Probably want to make it clear that the dye wasn't completely quenched (10-20% residual fluorescence) and this is what gives rise to high background (as opposed to unbound dye molecules)
...
% AMY wrt including more specifics: can you give examples from literature or application of this approach?  such as including a statement like: "for example, including XXX residues has been shown to increase the stability of the beta turn" such that modulation of residues can be used to tune the strength of the interaction.
...
% TODO maybe discuss the fact that this could abolish binding, and that screening will be necessary to restore it? (maybe put that in alternate approaches section?)
% TODO also discuss the possibility that these modifications will prohibit binding to endogenous machinery (that probably contributes to background)
...
% TODO more here about how the aptamer should be able to increase turn-on and that screening will also be done with previously developed probes.
