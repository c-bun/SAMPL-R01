...
% AMY I think its entirely appropriate to invoke this lncRNA here.  It seems like a good candidate if it interacts with other RNAs.

% AMY You could expand  on the "burning questions"....for example when and where this lnc RNA interacts with its interacting partners, the dynamics (for how long), what the complexes look like (would it be informative to look at 2 RNAs and a protein?  Are proteins part of the RNA "complexes"?)

\comment{I tried to set up a bit of a story here with the SNGH5 gene that will carry though the rest of the proposal. Let me know if this seems appropriate.}
...
% AMY I am wondering if "amide couplings" provides enough detail to reviewers to assess synthetic feasibility.  Do you need to indicate what functional groups are necessary on Cbl and the fluorophore and clarify that you can obtain / synthesize such molecules? Is it reasonable to show a model synthetic scheme?
...
% AMY define CuAAC before you use it.
...
% AMY Somewhere you should probably say that molecules will be purified by HPLC and structurally verified by small molecule NMR and mass spec.
...
\comment{I still need to work in here some statements about RNA imaging experiments being inherently ``sparse labeling.''}
...
% AMY Clarify that libraries will be virally transduced using Lentivirus at a low multiplicity of infection to ensure incorporation of a single library member per cell, a technique widely used in the Palmer Lab.
Cells that show elevated brightness relative to a fluorescent protein expression control will be sorted.
% AMY clarify: "fluorescence activated cell sorting, FACS" and make sure that our Cell sorting facility is part of your Facilities and Equipment sections.  If it's not there let me know and I will get the info for you.
...
% AMY Probably worth emphasizing that the advantage of a cellular screen like this is that you ensure the riboswitch aptamer maintains robust folding in mammalian cells.
...
% AMY Clarify that these are experiments readily carried out in the Palmer Lab (you can reference a few of our papers).
% AMY Regarding ITC, clarify that your goal is to ensure that the evolved aptamer / probe systems retain Kd values in the nM range.
...
% AMY We should clarify the type of cells here.
...
% AMY As an alternative approach you could include a statement like this: "While screening for high fluorescence in cells via FACS is the most efficient way to ensure robust turn-on, an alternative approach could involve alternating in vitro selection (SELEX) to identify RNA aptamers thattightly bind the modified probes (see aim 3 for experimental details) and in cellulo selection for fluorescence via FACS.
...
\comment{This is where I think the proposal breaks down a little bit. It's hard to discuss multicomponent imaging in a clear way, and even harder to design an experiment to validate the tool. Let me know what you think.}
...
% AMY I would emphasize dynamics...when, where and for how long...spatiotemporal dynamics...
...
%\textit{Probes will be spectrally distinguished through the use of distinct fluorophores, and different riboswitch sequences will be used to bind each structurally unique probe in a mutually-exclusive manner (Figure \ref{figure:aim3}B)}.
...
% AMY I think more chemical detail is warranted.  Can you include structures of the actual Cbl variants you hope to create?
...
If a robust pair of Riboglow probes is identified, a lncRNA will be imaged interacting with a putative binding partner mRNA. \textit{SNGH5} has been shown to promote colorectal cancer survival through stabilization of mRNA that promote tumor growth\cite{DamasSNHG5promotescolorectal2016}. Among these mRNA is the \textit{SPATS2} gene, which has a base pairing interaction in its 3'-UTR with \textit{SNGH5} exon 1. Matched Riboglow probes will be used to image this interaction.
% AMY I think this can be developed a bit more deeply to lay out exactly what experiment(s) you would conduct and what specific question you would be trying to address.  I can spend some time mid-day tomorrow doing a little research and try to offer some suggestions - but want to get this back to you for now.
...
\comment{I'm making the assumption here that single-molecule imaging capability will be retained after SELEX, which I think is a bit of a jump. As a protein engineer, I've now convinced myself that I would develop the probes in the reverse order: SELEX first (which is like hitting the aptamer with a hammer) and then FACS screen second (which is more gentle). I have toyed with switching the aims, but I can't think of a nice, non single-molecule technique for evaluating the multicomponent probes (mostly because I'm still clueless about the literature). Thoughts?}
% AMY I think the two screens are complementary and get at different functionalities.  I think it's wise to suggest that you can itterate the two approaches, but leave aim 2 more focused on the cell based screen and aim 3 more focused on the in vitro screen
